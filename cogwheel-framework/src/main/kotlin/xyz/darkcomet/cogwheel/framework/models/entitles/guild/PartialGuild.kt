...
internal fun GuildObject.toPartialGuildModel(): PartialGuild {
    return PartialGuild(
        ...
        splash = requireNonNullIfPresent(this, GuildObject::splash)?.let { DiscordImage.fromImageHash(it) },
        banner = requireNonNullIfPresent(this, GuildObject::banner)?.let { DiscordImage.fromImageHash(it) },
        features = requireNonNullIfPresent(this, GuildObject::features)?.map { GuildFeature.fromOrAdd(it) },
        icon = requireNonNullIfPresent(this, GuildObject::icon)?.let { DiscordImage.fromImageHash(it) },
